...
@section('page-sub','Input pengeluaran barang')
...
<form action="{{ route('stock-out.store') }}" method="POST" class="space-y-4">
    ...
    <div id="currentStock" class="hidden px-4 py-3 bg-amber-50 border border-amber-100 rounded-xl text-sm text-amber-700 flex items-center gap-2">
    ...
    <div class="grid grid-cols-2 gap-4">
        <div>
            ...
            <input type="number" name="quantity" value="{{ old('quantity') }}" min="1" id="qtyInput" placeholder="0"
                   class="w-full px-3.5 py-2.5 text-sm border border-slate-200 rounded-xl focus:ring-2 focus:ring-primary-500 outline-none" required>
        </div>
        <div>
            <label class="block text-sm font-semibold text-slate-700 mb-1.5">Tanggal Keluar <span class="text-red-500">*</span></label>
            <div class="relative">
                <input type="text" id="issuedDate" name="issued_date"
                       value="{{ old('issued_date', date('d/m/Y')) }}"
    ...
    <div>
        <label class="block text-sm font-semibold text-slate-700 mb-1.5">Tujuan / Keperluan</label>
        <input type="text" name="purpose" value="{{ old('purpose') }}" placeholder="Keperluan pengeluaran barang..."
               class="w-full px-3.5 py-2.5 text-sm border border-slate-200 rounded-xl focus:ring-2 focus:ring-primary-500 outline-none">
    </div>
    <div>
        <label class="block text-sm font-semibold text-slate-700 mb-1.5">Penerima</label>
        <input type="text" name="recipient" value="{{ old('recipient') }}" placeholder="Nama penerima..."
               class="w-full px-3.5 py-2.5 text-sm border border-slate-200 rounded-xl focus:ring-2 focus:ring-primary-500 outline-none">
    </div>
    <div>
    ...
    <div class="flex gap-3 pt-2">
        <button type="submit" class="px-6 py-2.5 bg-orange-600 text-white text-sm font-semibold rounded-xl hover:bg-orange-700 transition-colors">
            <i class="fa-solid fa-save mr-1.5"></i> Simpan
        </button>
        <a href="{{ route('stock-out.index') }}" class="px-6 py-2.5 bg-slate-100 text-slate-600 text-sm font-medium rounded-xl hover:bg-slate-200 transition-colors">Batal</a>
    </div>
</form>
</div>
</div>
